# Code is taken from https://github.com/nikitacs16/nlsi and updated
...
class Evaluation:
    ...
    def _get_f1_score(cls, prediction: List[str], ground_truth: List[str]) -> float:
        ...
        #         return 1.0
        #     return 0.0
        # precision = tp / len(set(prediction))
        # recall = tp / len(set(ground_truth))
        precision = cls._get_prec_score(prediction, ground_truth)
        recall = cls._get_rec_score(prediction, ground_truth)
    ...
    def _get_rec_score(cls, prediction: List[str], ground_truth: List[str]) -> float:
        ...
        if tp == 0:
            if len(ground_truth) == 0 and len(prediction) == 0:
                return 1.0
            return 0.0
        return tp / len(set(ground_truth))
